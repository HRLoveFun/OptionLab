...
class CorrelationValidator:
    ...
    def __init__(
        ...
        self.price_dynamic = (
            price_dynamic if price_dynamic is not None else PriceDynamic(ticker, start_date, frequency, end_date)
        )
    ...
    def generate_correlation_chart(self, corr_type: str = "return") -> str | None:
        ...
        try:
            ...
            if corr_1y is not None and not corr_1y.empty:
                ax.plot(corr_1y.index, corr_1y.values, color=COLOR_1Y, linewidth=1.5, label="1-Year Rolling", alpha=0.8)

            # Plot 5-year correlation
            if corr_5y is not None and not corr_5y.empty:
                ax.plot(corr_5y.index, corr_5y.values, color=COLOR_5Y, linewidth=1.5, label="5-Year Rolling", alpha=0.8)
