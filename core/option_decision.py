...
def _find_put_by_delta(puts_df: pd.DataFrame, target_delta: float, spot: float) -> pd.Series | None:
...
def build_candidate_matrix(
    analyzer: OptionsChainAnalyzer, candidate_deltas: list[float] | None = None, candidate_dtes: list[int] | None = None
) -> list[dict]:
...
def filter_and_rank(
    enriched: list[dict], min_dte: int, max_dte: int, min_ev: float = MIN_EV_THRESHOLD, min_vt: float = MIN_VEGA_THETA
) -> list[dict]:
...
def run_decision_process(
    ...
    for c in raw_matrix:
        ...
        if c is None:
            continue
        c = compute_ev(c, directional_conviction, vol_conviction, budget, time_horizon_days)
        enriched.append(c)
