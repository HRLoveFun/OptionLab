"""Listed-expiration calendar generation for the Option Pricing Matrix.

Domain:    Options Analysis — Expiration Calendar
Context:
  - Pure, deterministic computation — no network access. The caller supplies
    the reference date (and optionally the wall clock) so output stays stable
    in tests.
  ...
  - Feeds the dashboard "Option Pricing Matrix" tab: its columns are the
    expirations this module produces. Kept separate from
    :mod:`core.options.simulation.expiry` (the payoff maths) so neither module
    grows into a god file.
Contracts:
  - generate_expiry_calendar(ref, n_standard=12, n_daily=10, holidays=None,
                             now=None) -> list[dict]
Dependencies UPWARD:
  - (stdlib only: datetime, zoneinfo)
Dependencies DOWNWARD:
  - core.options.simulation, services.options.simulation, routes.options, tests
"""
...
def _nyse_holiday_set(years) -> set[dt.date]:
    ...
    for year in years:
        for fixed in (dt.date(year, 1, 1), dt.date(year, 6, 19), dt.date(year, 7, 4), dt.date(year, 12, 25)):
            wd = fixed.weekday()
...
def generate_expiry_calendar(
    ...
    """Build upcoming option expirations for an Option Pricing Matrix.
