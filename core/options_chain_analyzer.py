...
class OptionsChainAnalyzer:
    ...
    def plot_iv_smile(self, expiry: str) -> str | None:
        try:
            ...
            ax.axvline(self.spot, color="black", linestyle="-", linewidth=1, alpha=0.5, label=f"Spot {self.spot:.2f}")
    ...
    def plot_skew_analysis(self, expiry: str) -> str | None:
        try:
            ...
            atm_iv = float(puts.loc[(puts["strike"] - self.spot).abs().idxmin(), "impliedVolatility"]) * 100
    ...
    def plot_oi_volume_profile(self, expiry: str) -> str | None:
        try:
            ...
            ax1.axhline(self.spot, color="black", linestyle="--", linewidth=1.5, label=f"Spot {self.spot:.0f}")
            ax1.axhline(max_pain, color="purple", linestyle=":", linewidth=1.5, label=f"Max Pain {max_pain:.0f}")
            ...
            ax2.axhline(self.spot, color="black", linestyle="--", linewidth=1.5, label=f"Spot {self.spot:.0f}")
            ax2.axhline(max_pain, color="purple", linestyle=":", linewidth=1.5, label=f"Max Pain {max_pain:.0f}")
            ax2.set_xlabel("Volume  (Call +ve / Put −ve)")
            ax2.set_title("Volume Distribution")
            ax2.legend(fontsize=8)
            ax2.grid(axis="x", alpha=0.3)

            fig.suptitle(f"OI / Volume Profile — {expiry}  |  Spot: {self.spot:.2f}", fontsize=12)
            fig.tight_layout()
            return _fig_to_base64(fig)
        except Exception as e:
