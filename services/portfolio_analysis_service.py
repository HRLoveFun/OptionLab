...
class PortfolioAnalysisService:
    # Map alternate field names so callers (and ad-hoc API users) can pass
    # either the canonical schema or the more colloquial one.
    _OPT_TYPE_ALIASES = {
    ...
    def _normalize_position(pos: dict) -> dict:
        ...
        if "option_type" not in out:
            ...
            if combined in PortfolioAnalysisService._OPT_TYPE_ALIASES:
                out["option_type"] = PortfolioAnalysisService._OPT_TYPE_ALIASES[combined]
            elif kind and side:
                key = f"{side}_{kind}"  # e.g. "buy_call"
                if key in PortfolioAnalysisService._OPT_TYPE_ALIASES:
                    out["option_type"] = PortfolioAnalysisService._OPT_TYPE_ALIASES[key]
        ...
        if "option_type" in out and isinstance(out["option_type"], str):
            ...
            if ot in {"LC", "SC", "LP", "SP"}:
                out["option_type"] = ot
            elif ot.lower() in PortfolioAnalysisService._OPT_TYPE_ALIASES:
                out["option_type"] = PortfolioAnalysisService._OPT_TYPE_ALIASES[ot.lower()]
    ...
    def run(positions: list, account_size=None, max_risk_pct=2.0) -> dict:
        ...
        # WHY: Tester (and external callers) sometimes use kind/contracts/premium
        # instead of the strict option_type/quantity/price schema. Normalize
        # up-front so a bad payload returns a structured error rather than
        # a bare KeyError(\"option_type\").
        try:
            positions = [PortfolioAnalysisService._normalize_position(p) for p in (positions or [])]
