    ...
    def run(data: dict) -> dict:
        ...
        try:
            ...
            target_move_pct = _float_in(
                data.get('target_move_pct', -0.08), _MOVE_PCT_MIN, _MOVE_PCT_MAX,
                'target_move_pct')
            time_horizon_days = _int_in(
                data.get('time_horizon_days', 21), _HORIZON_MIN, _HORIZON_MAX,
                'time_horizon_days')
            directional_conviction = _float_in(
                data.get('directional_conviction', 0.5),
                _CONVICTION_MIN, _CONVICTION_MAX, 'directional_conviction')
            vol_conviction = _float_in(
                data.get('vol_conviction', 0.5),
                _CONVICTION_MIN, _CONVICTION_MAX, 'vol_conviction')
            vol_timing = (data.get('vol_timing') or 'MEDIUM').strip().upper()
            if vol_timing not in _VALID_VOL_TIMINGS:
                return {'status': 'error',
                        'message': f'vol_timing must be one of {_VALID_VOL_TIMINGS}'}
