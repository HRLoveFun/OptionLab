...
- **Sigma reference**: the column the row-header σ multiple is computed for — the one closest to 30D (`REF_DTE_DEFAULT`) by default. There is no `select`: the column **header** is the control, so it is `tabindex="0"`, activatable by click / Enter / Space, and carries `aria-current="true"` plus a persistent `.is-ref` mark on both the `<col>` and the `<th>`. The mark on the header is a `box-shadow` bar, because that header is an opaque sticky mask — a translucent fill would let scrolled rows show through.
- **Crosshair**: hovering a column tints it (via its `<col>`, so one class write beats 41); hovering (or focusing) never rewrites values. Only an explicit click or Enter on a column **header** promotes it to the sigma reference — clicking a *data cell* is a read, not a selection.
