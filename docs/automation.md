...
| `yfinance-throttle`      | Any new `yf.download` / `yf.Ticker(...)` call site not preceded by `yf_throttle()` or routed through `data_pipeline/providers/` (the chokepoint since ADR 0011 / batch B1). | Hard architectural invariant from ADR 0005.                                   |
...
| `sqlite-bypass`          | New `sqlite3.connect(` outside `data_pipeline/store/db.py`.                                                                              | Bypasses WAL pragmas (ADR 0003).                                              |
