...
class TestEnrichContract:
    def test_basic_enrichment(self):
        c = _make_contract()
        result = enrich_contract(c, budget=5000, spot_price=160.0, target_move_pct=-0.08)
    ...
    def test_payoff_calculation(self):
        ...
        result = enrich_contract(c, budget=5000, spot_price=160.0, target_move_pct=-0.10)
        d = result["derived"]
        max(150.0 - 144.0, 0)  # 6
...
class TestComputeEV:
    def test_positive_ev_high_conviction(self):
        ...
        c = compute_ev(c, directional_conviction=0.8, vol_conviction=0.7, budget=5000, time_horizon_days=21)
    ...
    def test_zero_conviction_negative_ev(self):
        ...
        c = compute_ev(c, directional_conviction=0.0, vol_conviction=0.0, budget=5000, time_horizon_days=21)
