...
from data_pipeline._state import (
    _QUERY_CACHE_TTL,
    _cache_get,
    _cache_invalidate,
    _cache_set,
...
class TestCooldown:
    @patch("data_pipeline.transform.processing.process_frequencies", return_value=PipelineResult(rows=5))
    @patch("data_pipeline.transform.cleaning.clean_range", return_value=PipelineResult(rows=5))
    @patch("data_pipeline.ingest.ohlcv.upsert_raw_prices", return_value=PipelineResult(rows=5))
    ...
    @patch(
        "data_pipeline.ingest.ohlcv.upsert_raw_prices", return_value=PipelineResult(ok=False, error="download_failed")
    )
...
class TestEnsureRangeInflightDedup:
    ...
    @patch("data_pipeline.transform.cleaning.clean_range", return_value=PipelineResult(rows=5))
    @patch("data_pipeline.ingest.ohlcv.upsert_raw_prices", return_value=PipelineResult(rows=5))
    @patch("data_pipeline.store.db.fetch_df")
    ...
    def test_sentinel_start_skips_backfill_when_db_has_coverage(self):
        ...
        DataService._ensure_range_memo.clear()
        with (
            patch("data_pipeline.store.db.fetch_df") as mock_fetch_df,
            patch("data_pipeline.ingest.ohlcv.upsert_raw_prices") as mock_dl,
        ):
